...
def augment_data2(dataset, dataset_labels):
    ...
    aug_factors = dict()
    for ccval in range(0, 2):  # values in [0,1]
        if num_augs_enabled == 0:
            continue
        aug_factors[ccval] = math.ceil((float(label_counts[1])/float(num_augs_enabled)) / float(label_counts[ccval]))
    print(aug_factors)

    maximg = {0: 11000, 1: 11000}
